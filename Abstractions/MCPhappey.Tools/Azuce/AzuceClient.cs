...
public sealed class AzuceClient
{
    ...
    public AzuceClient(HttpClient client)
    {
        _client = client;
    }
    ...
    public async Task<JsonNode?> GetAsync(string path, Dictionary<string, string?>? query, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new InvalidOperationException($"Azuce GET '{uri}' failed ({(int)response.StatusCode}): {raw}");
    ...
    public async Task<JsonNode?> PostAsync(string path, object payload, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new InvalidOperationException($"Azuce POST '{path}' failed ({(int)response.StatusCode}): {raw}");
